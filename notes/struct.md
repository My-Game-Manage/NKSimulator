...
1step分の処理をして、新しいstateを返す
必要になるのはParamと現在のState
...
- velocity: 速度      <= accel: 加速
- position: 位置      <= velocity * dt * K（係数）
- lane_p: 位置（横）
- stamina: 体力       <= remain_stamina: 体力残り

欲しい情報

- 馬の現在の位置（セクション情報／現在の距離／現在のレーン）
- 前方の馬（距離判別／いる・いない／いる場合の距離により変化）


- 次の行動（0: 移動せず／1: 外／2: 内）
- 次の戦術（0: 同速度／1: 速度アップ／2: 速度ダウン／3: スパート／4: ブレーキ）
...
基本はSTAYかOUTSIDE（外に出す）
内側はいない場合に内に寄せるか、そのままいくかを残りの距離で判断か（最終コーナー以降はそのままとか
道中で外に出てる場合は内に寄せる（レーンを1に近づける）
外に出すのは「抜く」判断の時。そうじゃないとSTAYかINかな

速度の戦術は基本は、巡航速度（ideal_speed）までは上げる。前にいる場合に速度調整。移動戦術による
...
- 前に馬がいるか？
  - いない        => STAY
  - いる          => STAY？MOVE？   => 基本は外側に避ける／コーナーではそのまま／脚質によってはそのまま

- 今のレーンは目標レーンか？
  - 目標レーン    => STAY
  - ではない      => MOVE（目標レーンの方に動く   => ただし最終コーナー以降はまっすぐ
...
- target_v: 目標速度｜フェーズにより目標速度が変化。これがlimited_max_speedになる
- accel: 加速度＞フェーズ（モード）により馬Paramの加速力に係数調整
- velocity: 速度（加速度から求める）｜加速度をdt分だけ追加する＝ current_velocity + accel * dt
- distance: 進む距離（速度から求める）｜速度をdt分だけ追加する＝ current_distance + new_velocity * dt
