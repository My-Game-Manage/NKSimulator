...
class RaceAnalyer:
    ...
    def update_ranks(race_snap: RaceSnapshot) -> RaceSnapshot:
        """現在の距離に基づき、全馬の順位を更新した新しいSnapshotを生成する"""
        # 既にゴールしている馬がいた場合の対処
    ...
    def update_time_at_600m(distance: float, race_snap: RaceSnapshot) -> RaceSnapshot:
        ...
        # 新しい辞書を作成して、元のレース状態を汚さないようにする
        horses = dict(race_snap.horses)
        is_updated = False
    ...
    def update_laptime_at_furlong(race_snap :RaceSnapshot) -> RaceSnapshot:
        ...
        for h_id, h_snap in horses.items():
            ...
            # 念のためリストの長さが足りているかチェック
            if current_distance - 1 < len(h_snap.laptimes):
                if h_snap.distance >= current_distance * 200 and h_snap.laptimes[current_distance - 1] <= 0.0:
                    # その地点のラップタイムを記録する
                    # リストを新しくコピーして作成
                    new_laptimes = list(h_snap.laptimes)
                    new_laptimes[current_distance - 1] = h_snap.elapsed_time

                    # replace で新しい HorseSnapshot を作り、新しい辞書に代入
                    update_snap = replace(h_snap, laptimes=new_laptimes)
                    horses[h_id] = update_snap
                    is_updated = True
        ...
        for h_id, h_snap in horses.items():
            for idx in range(len(checkpoints)):
                ...
                if h_snap.distance >= check_dist and h_snap.checkpoint_ranks[idx] <= 0:
                    # その地点での順位を記録する
                    # リストを新しくコピーして作成
                    new_checkpoint_ranks = list(h_snap.checkpoint_ranks)
                    new_checkpoint_ranks[idx] = race_snap.ranks[h_id]

                    # replace で新しい HorseSnapshot を作り、新しい辞書に代入
                    update_snap = replace(h_snap, checkpoint_ranks=new_checkpoint_ranks)
                    horses[h_id] = update_snap
                    is_updated = True
