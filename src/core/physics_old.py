...
from src.constants.fields import HorseEnvField, HorseSnapField
...
def get_dist_to_front_context(horse_id: str, horses: dict[str, HorseSnapshot]) -> dict:
    ...
    context = {
        HorseSnapField.DIST_TO_FRONT: DIST_TO_FRONT_MAX,
        HorseSnapField.DIST_TO_FRONT_LEFT: DIST_TO_FRONT_MAX,
        HorseSnapField.DIST_TO_FRONT_RIGHT: DIST_TO_FRONT_MAX,
        HorseSnapField.DIST_TO_SIDE_LEFT: DIST_TO_FRONT_MAX,
        HorseSnapField.DIST_TO_SIDE_RIGHT: DIST_TO_FRONT_MAX,
    }
    ...
    for h_id, other in horses.items():
        ...
        if 0 < d_dist < DIST_FRONT_RANGE:
            if abs(d_lane) < DIST_RIGHT_IN_FRONT: # 真前
                context[HorseSnapField.DIST_TO_FRONT] = min(context[HorseSnapField.DIST_TO_FRONT], d_dist)
            elif -DIST_DIAGONALLY_IN_FRONT < d_lane <= -DIST_RIGHT_IN_FRONT: # 左斜め前
                context[HorseSnapField.DIST_TO_FRONT_LEFT] = min(context[HorseSnapField.DIST_TO_FRONT_LEFT], d_dist)
            elif DIST_RIGHT_IN_FRONT <= d_lane < DIST_DIAGONALLY_IN_FRONT: # 右斜め前
                context[HorseSnapField.DIST_TO_FRONT_RIGHT] = min(context[HorseSnapField.DIST_TO_FRONT_RIGHT], d_dist)
        
        # 真横 (並走状態の検知)
        if abs(d_dist) < DIST_BESIDE_RANGE:
            if -DIST_BESIDE_RANGE < d_lane < DIST_BESIDE_RANGE_MIN: context[HorseSnapField.DIST_TO_SIDE_LEFT] = min(context[HorseSnapField.DIST_TO_SIDE_LEFT], abs(d_lane))
            if DIST_BESIDE_RANGE_MIN < d_lane < DIST_BESIDE_RANGE: context[HorseSnapField.DIST_TO_SIDE_RIGHT] = min(context[HorseSnapField.DIST_TO_SIDE_RIGHT], abs(d_lane))
